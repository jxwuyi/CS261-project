...
static void test_paillier_dot_product() {
  cout << "Test Paillier Dot Product ..." << flush;
  ...
  for (int i = 0; i < kVectorSize; i++) {
    ...
    plaintexts.emplace_back(std::to_string(text).c_str());
    ciphertexts.emplace_back(p.encrypt(text));
    dotsum += c * text;
  };
  ...
  assert(pp.decrypt(result) == expected);
  cout << " passed" << endl;
